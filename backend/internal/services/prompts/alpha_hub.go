...
- smart_money (per market, last 1h): wallet tiers = Gold (win_rate>=0.7 & realized_pnl_usd>0), Silver (0.6–0.7 & pnl>=0), Bronze (else) with net_buy_usd_1h, net_sell_usd_1h, buys_count, sells_count, whale_hits_count, avg_entry_vs_mid_bps, wallets_considered
...
- resolution_risk: {ambiguity_score (0-1, higher=worse), blockers:[...]}
- data_freshness_secs
...
3) Score (0–100): smart_money 35%, fundamentals/rules 20%, momentum 15%, liquidity/spread 15%, news 10%, resolution_risk penalty up to 20.
4) Return probability_yes in [0,1] that can diverge from price when justified; explain divergence briefly. Conviction: High≥75, Medium 50–74, Low<50. Labels: ending_soon|momentum|value|event_risk|illiquid.
