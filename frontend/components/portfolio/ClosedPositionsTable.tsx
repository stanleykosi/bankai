...
/**
 * @description
 * Closed positions history table for the connected wallet.
 */
...
export function ClosedPositionsTable({
  ...
  const summarized = useMemo(() => {
    if (!positions) return { totalPnL: 0 };
    return {
      totalPnL: positions.reduce((sum, pos) => sum + pos.realizedPnl, 0),
    };
  }, [positions]);
      ...
      <Card className="border-border/60 bg-card/70">
        <CardHeader className="pb-3">
          <div>
            ...
            <h3 className="text-lg font-semibold text-foreground">Closed Positions</h3>
          </div>
        </CardHeader>
  ...
  const totalPnLColor =
    summarized.totalPnL >= 0 ? "text-emerald-400" : "text-rose-400";
      ...
      <CardHeader className="pb-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
            <span className="font-mono">{positions.length} closes</span>
            <span className="inline-flex items-center gap-2 rounded-full border border-border/60 bg-background/40 px-3 py-1 font-mono">
              ...
              <span className={totalPnLColor}>{formatCurrency(summarized.totalPnL)}</span>
            </span>
          </div>
        </div>
      </CardHeader>
