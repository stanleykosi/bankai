...
import { FillsPanel } from "@/components/terminal/FillsPanel";
...
export function OrdersPanel() {
  ...
  const allVisibleSelected =
    ...
    activeOrders.every((order) => selected.has(order.clob_order_id));
  ...
  const toggleAll = () => {
    if (allVisibleSelected) {
      setSelected((prev) => {
        const next = new Set(prev);
        activeOrders.forEach((order) => next.delete(order.clob_order_id));
        return next;
      });
    } else {
      setSelected((prev) => {
        const next = new Set(prev);
        activeOrders.forEach((order) => next.add(order.clob_order_id));
        return next;
      });
    }
  };
      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
        <div className="flex items-center gap-3">
          ...
          <div className="flex gap-2">
            ...
            <Button
              size="sm"
              variant={activeTab === "fills" ? "default" : "secondary"}
